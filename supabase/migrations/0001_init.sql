-- Schema initialization for Travel Group Buying app
-- Creates tables and RPC aligned with current codebase

begin;

-- Users (profile table referencing auth.users by id)
...
create index if not exists idx_group_buyings_product on public.group_buyings(product_id);
create index if not exists idx_group_buyings_creator on public.group_buyings(creator_id);
...
create table if not exists public.wallet_transactions (
  ...
  user_id uuid not null references public.users(id) on delete cascade,
  type text not null check (type in ('recharge','payment','refund','withdraw')),
...
-- Chat Messages (used by GroupChatRealTime, uses column `message`)
